...
const Index = props => {
  ...
  const handleAddRelationShip = newData => {
    setVisibleAddRelationShip(false);
    const duplicatedItems = newData.filter(itemNew => {
      return dataRelationShip.some(
        itemExisting =>
          itemExisting.MoiQuanHe === itemNew.MoiQuanHe &&
          itemExisting.HoTen === itemNew.HoTen &&
          moment(itemExisting.NgaySinh).isSame(itemNew.NgaySinh, 'day'),
      );
    });
    ...
    if (duplicatedItems.length > 0) {
      Toast.show({
        type: 'error',
        props: {message: t('existRelationShip')},
      });
      return;
    }
    setDataRelationShip([...dataRelationShip, ...newData]);
  };

  const handleDeleteRelation = data => {
    setDataRelationShip(prevDataRelationShip =>
      prevDataRelationShip.filter(
        item =>
          !(
            item.MoiQuanHe === data.MoiQuanHe &&
    ...
    const fields = [
      {
        ...
        message: t('emptyLastName'),
      },
      {
        value: firstName,
        setError: setFirstNameError,
        message: t('emptyFirstName'),
      },
      ...
      {value: address, setError: setAddressError, message: t('emptyAddress')},
      {
        ...
        message: t('emptyBirthday'),
      },
      {
        value: citySelected,
        setError: setCitySelectedError,
        message: t('emptyCitySelected'),
      },
      {
        value: zipcodeSelected,
        setError: setZipcodeSelectedError,
        message: t('emptyZipcodeSelected'),
      },
    ];
  ...
  const handleAddNewCustomer = () => {
    ...
    const params = {
      ...
      gioitinh: gender,
      ghichu: note,
      dsmoiquanhe: dataRelationShip,
    };
    fetchData(dispatch, 'addBuyer', params, res => {
      if (res?.success === true) {
    ...
    <MySafeAreaView style={styles.container}>
      ...
      <MyAvoidView>
        <MyView style={styles.content}>
          ...
          <Gender value={gender} onSelect={value => setGender(value)} />
          <DatePicker
        ...
        <MyView style={styles.wrapRelation}>
          <MyText style={styles.textTitleRelation}>
            {t('listRelationship')}
          </MyText>
          {dataRelationShip &&
            dataRelationShip.length > 0 &&
            dataRelationShip.map((data, index) => {
              return (
                <MyView key={index} style={styles.wrapInfoRelation}>
                  <MyTouchableOpacity
                    style={styles.wrapIconClear}
                    onPress={() => handleDeleteRelation(data)}>
                  ...
                  <MyView style={styles.wrapInfo}>
                    ...
                    <MyText style={styles.textInfo}>
                      {data?.HoTen} |{' '}
                      {genderText.find(x => x.gender === data?.GioiTinh)?.label}
                    </MyText>
                    <MyText style={styles.textInfo}>
                      {data?.NgaySinh
      ...
      <Bottom
        ...
        titleBtn2={t('add')}
        onPress1={() => setVisibleAddRelationShip(true)}
        onPress2={() => handleAddNewCustomer()}
        typeBtn2={0}
      />
      <ModalAddRelationShip
        isVisible={visibleAddRelationShip}
        onClose={() => setVisibleAddRelationShip(false)}
        onConfirm={data => handleAddRelationShip(data)}
      />
    </MySafeAreaView>
  );
};
