...
const stringBillHtmlIOS = ({HoTen, data}) => {
  //DATA ORDER
    ...
    return data
      .map(item => {
        const {MaSanPham, TenSanPham, NgayHetHanBaoHanh, SoLuong, ThanhTien} =
          item;
        const barcodeHTML = `<div class="Barcode" style="padding: 0px; overflow: auto; width: 200px;">
                                 ${genBarCodeHtml(MaSanPham)}
                               </div>`;
        const productHTML = `<div style="font-size: 12px;">${TenSanPham}</div>`;
        const warranty =
  ...
  <body cz-shortcut-listen="true">
    ...
    @media only screen and (max-device-width: 844px) and (min-device-height: 390px) and (-webkit-device-pixel-ratio: 3) {
      ...
      <div id="dvHoaDon1" class ="page-break" >
        <table id="dvHoaDonHeader" cellpadding="0" style="margin-top: 10px">
          <tbody>
            <tr>
              <td>
                <img style="width: 100px;" class="logoHoaDon" src="https://uat.phuckhanggem.com/_imageslibrary/stores/0.png">
              </td>
              <td style="text-align: center;">
                <div style="font-size: 36px;font-weight: bold">PHIẾU MUA HÀNG</div>
                <div style="font-size: 20px;">Ngày mua hàng: <span id="txtNgayDatHang">${NgayDatHang}</span>
                </div>
              </td>
              <td style="text-align: left">
                <div class="QRCode">
                  <img style="width: 75px;" class="logoHoaDon" src=${qrDataURL}>
                </div>
              </td>
            </tr>
          </tbody>
        ...
        <hr>
        <table style="font-size: 20px" cellpadding="0" style="margin-top: 10px">
          <tbody>
            <tr>
              <td>Khách hàng: <span class="txtpan" style="font-size: 18px; font-weight: bold">${HoTenNguoiNhan}</span>
              </td>
              <td>Điện thoại: <span class="txtpan" style="font-size: 18px; font-weight: bold">${DienThoaiKhachHang}</span>
              </td>
            </tr>
            <tr>
            ...
            <tr>
              <td colspan="2" style="font-size: 18px; font-weight: bold; padding: 2px 0" id="txtDiaChi">${DiaChiNhan}</td>
            </tr>
            <tr>
              <td>Nhận hàng: <span class="txtpan" style="font-size: 18px; font-weight: bold">${TenHinhThucNhanHang}</span>
              </td>
              <td>Thanh toán: <span class="txtpan" style="font-size: 18px; font-weight: bold">${TenHinhThucThanhToan}</span>
              </td>
            </tr>
            <tr>
        ...
        <table id="tSanPham" style="font-size: 18px;margin-top: 10px">
          <tr>
        ...
        <table >
          <tbody>
            <tr>
              <td style="font-size: 13px; font-weight: bold; font-style: italic; color: var(--color1)"> * Hàng mua rồi miễn đổi trả. Vui lòng kiểm tra kỹ trước khi rời cửa hàng. </td>
              <td style="text-align: right;font-size: 18px" > * <span id="txtMaDonHang">${MaDonHang}</span> | <span id="txtSoTrang">1</span>/ <span class="txtTongSoTrang">1</span>
              </td>
            </tr>
            <tr>
              <td style="text-align: right;font-size: 18px" colspan="2"> Người lập phiếu: <span style="font-weight: bold">${HoTen}</span>
              </td>
            </tr>
          </tbody>
        </table >
      ...
      <div class="dvHoaDon2">
        <table cellpadding="0" style="font-size: 20px;position: relative; top: 10px;">
          <tbody>
            <tr class="trtitle">
...
export default stringBillHtmlIOS;
